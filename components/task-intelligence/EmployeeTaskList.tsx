...
function isPersonalOverdue(task: PersonalTask): boolean {
  if (task.atlas_status === "done" || task.atlas_status === "cancelled")
    return false;
  if (!task.due_date) return false;
  return new Date(task.due_date).getTime() < Date.now();
}

/** SOP list: hide finished rows (done / cancelled still appear in “Completed (past 7 days)” when applicable). */
function isSopListVisible(task: PersonalTask): boolean {
  return task.atlas_status !== "done" && task.atlas_status !== "cancelled";
}
...
export function EmployeeTaskList({
  ...
  const dailySopVisible = useMemo(
    () => safePersonal.dailySop.filter(isSopListVisible),
    [safePersonal.dailySop],
  );
