...
function getLastTurn(session: BotSession): { text: string; lastRole: string } {
  ...
  if (!turns || turns.length === 0) return { text: "No messages yet", lastRole: "" };
  const last = turns[turns.length - 1];
  // If there's an inbound message, the customer spoke last
  const lastRole = last?.in ? "user" : "assistant";
...
function sortSessions(sessions: BotSession[]): BotSession[] {
  return [...sessions].sort((a, b) => {
    const { lastRole: aRole } = getLastTurn(a);
    const { lastRole: bRole } = getLastTurn(b);
    // Sessions awaiting reply (last message from user) float to top
    const aAwaiting = aRole === "user" ? 0 : 1;
    const bAwaiting = bRole === "user" ? 0 : 1;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-0.5">
        ...
        {sorted.map((session) => {
          ...
          const { lastRole } = getLastTurn(session);
          const awaitingReply = lastRole === "user";
          const { text: previewText } = getLastTurn(session);
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <div className="flex items-center gap-1.5 min-w-0">
                  {/* Agent mode dot */}
                  {isHandedOff && (
                    <span className="w-2 h-2 rounded-full bg-success shrink-0" title="Agent mode" />
                  )}
                  {/* Awaiting reply dot */}
                  {awaitingReply && !isHandedOff && (
                    <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" title="Customer waiting" />
                  )}
