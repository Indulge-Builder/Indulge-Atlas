...
lib/services/             Internal business services (not component-facing)
  ...
  academyEvaluator.ts     Academy session close → Haiku structured scoring → training_reviews
                          (service role; NOT "use server")
...
- **Never use Opus, Sonnet, or Fable — Atlas is Haiku-only.** Every Anthropic call in this repo runs `claude-haiku-4-5-20251001`: Elia chat, client summary, WhatsApp profile analysis, Freshdesk ticket AI, the Gupshup bot, and all four Academy calls (persona, AI-assist estimator, evaluator, ticket reviewer). This is a standing product decision, not a per-task tuning knob — do not "upgrade" a call because the task looks hard. All the API keys bill **one org cap**, so a single higher-tier surface starves every other one. If quality is short, fix the prompt, bound the input, or move the judgment into code — not the model tier. Pinned by `__tests__/academy-evaluator.test.ts` ("uses Haiku for every Academy call")
...
**Models (Atlas today):** **Haiku-only — never Opus, Sonnet or Fable.** Every production call uses **`claude-haiku-4-5-20251001`** via REST (`api.anthropic.com/v1/messages`). No streaming outside Academy. Chat: `max_tokens` 1024; summary: 300; WhatsApp profile analysis: 2000; Gupshup bot: 512. Note Haiku 4.5 **rejects `output_config.effort`** outright (400 before the prompt is read) — never attach it.
...
| **Evaluator** (the scorer) | `runAcademyEvaluation` (`lib/services/academyEvaluator.ts`) on close — **`claude-haiku-4-5-20251001`**, structured JSON output, `max_tokens` 6000, no `effort` (Haiku 400s on it), idempotent on `session_id`. Six dimensions 1–5, `factual_accuracy` weighted 1.5; **`overall` is computed in code** (`computeOverall`), never asked of the model |
| **Ticket reviewer** | `runAcademyTicketReview` (`lib/services/academyTicketReview.ts`) — same model as the evaluator, `max_tokens` 4000. Quality + pass are recomputed in code (`computeTicketQuality`, `decidePassed`); a model "pass" can only be downgraded |
| **Model policy** | **Academy is single-tier: every Anthropic call it makes is Haiku 4.5** — persona, AI-assist estimator, evaluator, ticket reviewer. All four ids live in `lib/academy/models.ts`; nothing else in Academy names a model |
