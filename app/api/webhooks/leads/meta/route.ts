...
export async function POST(request: NextRequest) {
  ...
  const authError = verifyBearerSecret(request, "PABBLY_META_SECRET");
  if (authError) {
    console.warn(
  ...
  // ── Dynamic mapping engine (database-driven) ───────────────────────────────
  // If managers have configured field mappings for this channel, apply them now.
  // hasMappings=false means no rows saved yet → fall through to hardcoded logic.
  const { hasMappings, mappedFields, unmappedFormData } =
    await applyFieldMappings("meta", rawBody);
  ...
  if (hasMappings) {
    ...
    const result = await processAndInsertLead(dynamicPayload, "meta");
    if (result.success) {
      return NextResponse.json(
        {
          success: true,
  ...
  // ── End dynamic engine — fall through to hardcoded parsing ─────────────────
