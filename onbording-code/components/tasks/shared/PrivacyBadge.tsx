...
interface PrivacyBadgeProps {
  ...
  className?: string;
  /**
   * When false, render only Tooltip (no Provider). Caller must wrap the list in
   * a single TooltipProvider to avoid dozens of nested providers (Radix + list remounts).
   */
...
export function PrivacyBadge({
  ...
  if (withProvider) {
    return <TooltipProvider delayDuration={200}>{inner}</TooltipProvider>;
  }
  return inner;
}
